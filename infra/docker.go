...
// InstallDocker install docker on node
func InstallDocker(node Node) error {
	sudo := ""
	if node.User != "root" {
		sudo = "sudo"
	}
	...
	sshPort := GetSSHPort()
	ssh := sshOperator.New(node.IP).WithUser(node.User).WithKey(sshKeyFile).WithPort(sshPort)
...
func StartDockerd(node Node) error {
	installCmd := "dockerd >/dev/null 2>&1 & sleep 2"
	sshKeyFile, _ := GetSSHKeyFile()
	sshPort := GetSSHPort()
	ssh := sshOperator.New(node.IP).WithUser(node.User).WithKey(sshKeyFile).WithPort(sshPort)
...
func StartFxAgent(node Node) error {
